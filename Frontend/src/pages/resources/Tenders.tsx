  ...
  useEffect(() => {
    ...
    const loadDocuments = async () => {
      try {
        ...
        if (!mounted) return;
        setDocuments(items.filter((item) => Boolean(item.download_url) && item.section === "tenders"));
        setError(null);
      } catch (loadError) {
        if (!mounted) return;
        setError(loadError instanceof Error ? loadError.message : "Failed to load tenders.");
      } finally {
      ...
      <main className="min-w-0 flex-1 py-12 md:py-16">
        <section className="container max-w-5xl mx-auto px-4">
          <div className="mb-3 md:mb-4">
            ...
            <h1 className="text-3xl md:text-4xl font-extrabold text-foreground">Tenders</h1>
            <p className="mt-2 text-sm md:text-base leading-relaxed text-muted-foreground">
              View current procurement opportunities and tender notices published by BOCRA.
            </p>
          </div>
          ...
          {loading ? (
            ...
            <p className="text-sm text-muted-foreground">No tenders are available at this time. Check back later.</p>
          ) : (
