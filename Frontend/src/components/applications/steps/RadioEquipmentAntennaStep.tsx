...
export default function RadioEquipmentAntennaStep({ data, onChange, licenceType, readOnly }: StepProps) {
  ...
  // Private radio doesn't need this step (equipment is in the annexure tables)
  if (isPrivateRadio) {
    return (
      <StepContainer
        title="Equipment & Antenna"
        description="Equipment details are captured in the station tables from the previous step."
      >
        <div className="py-8 text-center text-muted-foreground text-sm">
          Equipment and antenna details for Private Radio licences are recorded in the repeater, base, and mobile station tables in the previous step.
        </div>
      </StepContainer>
    );
  }
  ...
  return (
    <StepContainer
      title="Equipment & Antenna"
      description="Provide details about the radio equipment and antenna to be used. You may need to consult your supplier."
    >
      {/* Equipment Type */}
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {EQUIPMENT_TYPES.map((t) => (
            <button
              ...
              type="button"
              disabled={readOnly}
              onClick={() => update("equipment_type", t)}
              className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                data.equipment_type === t
      ...
      <FormSection title="Equipment Details">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <FormField label="Frequency Range" name="frequency_range" value={data.frequency_range || ""} onChange={update} readOnly={readOnly} placeholder="e.g. 150-170" />
